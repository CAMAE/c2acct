...
export default async function FirmInsightDetailPage({
  ...
  if (!registryMatch) {
    redirect("/firm/insights");
  }

  const data = await getViewerIntelligencePageData();
  if (!data) {
    redirect("/login?callbackUrl=%2Ffirm%2Finsights");
  }
  ...
  return (
    <InsightDetailPage
      roleLabel="Firm insight detail"
      backHref="/firm/insights"
      backLabel="Back to firm insights"
      title={registryMatch.card.title}
      subtitle="Firm detail pages remain tied to the canonical self-owned lane. No external-review semantics are widened here."
