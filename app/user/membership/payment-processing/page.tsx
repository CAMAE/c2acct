...
export const metadata = {
  title: "Individual Membership Payment Processing | C2Acct",
  description: "Individual payment-processing handoff for PAT membership.",
};

function getSurfaceMessage(state: string | undefined) {
  if (state === "canceled") {
    return "The provider checkout was canceled before completion. No individual membership upgrade was finalized.";
  }

  return null;
}

export default async function UserMembershipPaymentProcessingPage({
  searchParams,
}: {
  ...
  async function startBillingSession(formData: FormData) {
    ...
    if (!parsed.success) {
      redirect(`/user/membership/payment-processing?plan=${selectedPlan.toLowerCase()}&error=invalid`);
    }
    ...
    if (!result.ok) {
      redirect(`/user/membership/payment-processing?plan=${selectedPlan.toLowerCase()}&error=${result.reason}`);
    }
  ...
  return (
    <MembershipPaymentProcessingPanel
      ...
      priceSummary={getConfiguredPriceSummary("individual", selectedPlan)}
