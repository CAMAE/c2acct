...
export const metadata = {
  title: "Vendor Membership Payment Processing | C2Acct",
  description: "Vendor payment-processing handoff for PAT membership.",
};

function getSurfaceMessage(state: string | undefined) {
  if (state === "canceled") {
    return "The provider checkout was canceled before completion. No membership upgrade was finalized.";
  }

  return null;
}

export default async function VendorMembershipPaymentProcessingPage({
  searchParams,
}: {
  ...
  async function startBillingSession(formData: FormData) {
    ...
    if (!parsed.success) {
      redirect(`/vendor/membership/payment-processing?plan=${selectedPlan.toLowerCase()}&error=invalid`);
    }
    ...
    if (!result.ok) {
      redirect(`/vendor/membership/payment-processing?plan=${selectedPlan.toLowerCase()}&error=${result.reason}`);
    }
  ...
  return (
    <MembershipPaymentProcessingPanel
      ...
      priceSummary={getConfiguredPriceSummary("vendor", selectedPlan)}
