...
export default function InsightCatalog({ items }: Props) {
  const available = items.filter((item) => item.unlocked);
  const pending = items.filter((item) => !item.unlocked);

  return (
    <div className="grid gap-6 lg:grid-cols-[1.35fr_0.95fr]">
      <div className="rounded-[22px] border border-[var(--shell-border)] bg-white p-5">
        <div className="text-sm font-semibold text-[var(--shell-ink)]">Available now</div>
        <div className="mt-4 grid gap-4">
          {available.length === 0 ? (
            <div className="text-sm text-[var(--shell-muted)]">No insights are unlocked yet for this subject.</div>
          ) : (
            available.map((item) => (
              <article key={item.title} className="rounded-[18px] border border-[var(--shell-border)] bg-[var(--shell-panel-soft)] p-4">
      ...
      <div className="rounded-[22px] border border-[var(--shell-border)] bg-white p-5">
        ...
        <div className="mt-4 grid gap-4">
          {pending.length === 0 ? (
            <div className="text-sm text-[var(--shell-muted)]">All current Pro membership insights are available.</div>
          ) : (
            pending.map((item) => (
              <article key={item.title} className="rounded-[18px] border border-[var(--shell-border)] bg-[var(--shell-panel-soft)] p-4 opacity-85">
                <div className="flex items-start justify-between gap-3">
                  <div>
