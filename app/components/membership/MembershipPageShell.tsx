...
import type { buildMembershipBillingSummary } from "@/lib/billing";
    ...
    <div className="space-y-8">
      ...
      <section className="pat-card p-8">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3 xl:grid-cols-5">
          ...
          <div className="pat-soft-panel p-4 text-sm leading-6 text-[var(--shell-muted)]">
            Billing: <span className="font-semibold text-[var(--shell-ink)]">{billingSummary?.providerLabel ?? "No live billing profile yet"}</span>
          </div>
          <div className="pat-soft-panel p-4 text-sm leading-6 text-[var(--shell-muted)]">
        ...
        {billingSummary?.pendingPlan ? (
          <div className="mt-4 rounded-[18px] border border-amber-200 bg-amber-50/90 p-4 text-sm leading-6 text-amber-900">
            Pending checkout: {formatMembershipValue(billingSummary.pendingPlan)} is still open.
            {billingSummary.pendingCheckoutCreatedAt
              ? ` Started ${billingSummary.pendingCheckoutCreatedAt.toLocaleString()}.`
              : ""}
          </div>
        ) : null}
        {checkoutNotice ? (
