...
type Props = {
  ...
  /** Kept for the draft contract (resume position); the page is one screen now. */
...
export default function VendorProductAssessmentClient({
  ...
  // A3 (route atlas box): ONE page, every question. The page plan still drives the
  // question sets and the draft's page bookkeeping (so what the server receives is
  // what it received before); the pager itself is gone. Scored and open-ended
  // questions stack in plan order under their section headers, numbered
  // continuously.
  ...
  const stackedQuestions = useMemo(
    () =>
      pages
        .flatMap((page) => page.entries.filter(isQuestionEntry).map((entry) => entry.question))
        .filter((question) => question.moduleKind !== "general"),
    [pages]
  );
  const stackedGroups = useMemo(() => groupQuestionsBySection(stackedQuestions), [stackedQuestions]);
  const questionNumberById = useMemo(
    () => new Map(stackedQuestions.map((question, index) => [question.id, index + 1])),
    [stackedQuestions]
  );
  ...
  const missingCount = totalCount - answeredCount + (selectedUtilityKeys.length > 0 ? 0 : 1);
  ...
  const saveDraft = useCallback(
      ...
      try {
        await fetch("/api/vendor/product-assessment/draft", {
          ...
          body: JSON.stringify({
            ...
            answers,
            // One screen: resume position is always the top. totalPages still
            // reflects the plan so the draft contract is unchanged.
            currentPage: clampPageIndex(1, totalPages),
            totalPages,
          }),
        });
    ...
    [answers, openEndedAnswers, productId, profile, selectedUtilityKeys, totalPages]
  );
    ...
    <div className="space-y-8">
      <section ref={topCardRef} className="pat-card p-8">
        ...
        <p className="mt-4 max-w-3xl text-base leading-7 text-[var(--shell-muted)]">
          Tell PAT what this product does, answer every question on this one page, and submit the finished response set for this product&apos;s current-state signal.
        </p>
        <div className="mt-6 grid gap-4 md:grid-cols-4">
          ...
          <div className="pat-soft-panel p-4 text-sm leading-6 text-[var(--shell-muted)]">
            Remaining: <span className="font-semibold text-[var(--shell-ink)]">{missingCount}</span>
          </div>
          <div className="pat-soft-panel p-4 text-sm leading-6 text-[var(--shell-muted)]">
        ...
        <div className="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
          <section className="pat-soft-panel p-5">
            ...
            <div className="mt-3 text-2xl font-semibold tracking-tight text-[var(--shell-ink)]">
              One page, every question
            </div>
            <p className="mt-3 text-sm leading-6 text-[var(--shell-muted)]">
              Product profile, feature declaration, scored questions and written follow-ups are all below. Every answer saves as you go.
            </p>
          ...
          <section className="pat-soft-panel p-5">
            ...
            <p className="mt-3 text-sm leading-6 text-[var(--shell-muted)]">
              Answer each question honestly, in any order; the progress panel shows what is still open before you submit.
            </p>
            <div className="mt-5 grid gap-3 text-sm leading-6 text-[var(--shell-muted)]">
              <div>
                ...
                confirm the right features, answer the scored questions, then finish the written follow-ups.
              </div>
              <div>
      ...
      {!questionLoadSafe ? (
        <section className="pat-card border-amber-200 bg-amber-50/90 p-6 text-sm leading-6 text-amber-950">
          ...
          <p className="mt-3">
            This feature selection generates {questionLoad.totalQuestionCount} questions, above the {questionLoad.maxBrowserSessionQuestionCount}-question browser-session guard. Narrow the feature scope before continuing so the assessment remains usable in one pilot QA session.
          </p>
        </section>
      ) : null}
      ...
      {stackedGroups.length > 0 ? (
        <section className="space-y-4" data-testid="vendor-assessment-stacked">
        ...
        <section className="pat-card p-6 text-sm leading-6 text-[var(--shell-muted)]">
          Declare at least one feature above to open the scored questions and written follow-ups.
        </section>
      )}
