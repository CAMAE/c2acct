...
export default async function UserMembershipCheckoutPage({
  ...
  async function startCheckout(formData: FormData) {
    ...
    if (!hasElevatedConfirmation(formData, ELEVATED_ACTION.MEMBERSHIP_CHECKOUT)) {
      redirect(`/user/membership/checkout?plan=${requestedPlan.toLowerCase()}&billing=confirmation_required`);
    }
    ...
    if (result.ok) {
      redirect(`/user/membership?checkout=${requestedPlan.toLowerCase()}&billing=scaffold&reason=${result.billingDisabledReason ?? "billing_disabled"}`);
    }

    redirect(`/user/membership/checkout?plan=${requestedPlan.toLowerCase()}&billing=unavailable&reason=${result.reason}`);
  }
