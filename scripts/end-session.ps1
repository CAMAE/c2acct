...
if (-not (Test-Path "docs/audit/run_audit.ps1")) {
  throw "Missing docs/audit/run_audit.ps1"
}

# 1) Run audit
powershell -NoProfile -ExecutionPolicy Bypass -File docs/audit/run_audit.ps1 -OutFile $auditOut

# 2) Write session log (repairs summary since last commit)
...
# 3) Append pointer to plan (no manual edits)
