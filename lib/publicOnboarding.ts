...
/**
 * Built per call rather than held as a module const: the vendor hero body reads
 * the resolved vertical's lexicon (class d, VERTICAL-READINESS-AUDIT-2026-08
 * §2), and a const would freeze that at import time. Flag off, `lexicon()`
 * returns the accounting literals with no pack load.
 */
function buildAudienceConfigs(): Record<PublicOnboardingAudience, AudienceBaseConfig> {
  return {
    vendor: {
      audience: "vendor",
      ...
      heroBody:
        `Map your product evidence, complete the product assessment, then use PAT to understand where the product fits the ${lexicon("firmMarket")}.`,
...
export function getPublicOnboardingConfig(audience: PublicOnboardingAudience) {
  return buildAudienceConfigs()[audience];
}
