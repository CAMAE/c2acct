...
- Auth mode is `github` for the canonical operator runtime. Local-review mode is allowed only for loopback proof with `PAT_ENABLE_LOCAL_REVIEW_AUTH=1`, `PAT_LOCAL_REVIEW_PASSWORD`, an Auth.js secret, and no public/non-loopback values in `AUTH_URL`, `NEXTAUTH_URL`, `PAT_LOCAL_ORIGIN`, `MAC_MINI_PUBLIC_ORIGIN`, `NEXT_PUBLIC_APP_URL`, or `PAT_PUBLIC_BASE_URL`. The documented local-review password spelling for local QA is `pat-local-review` with hyphens.
- Pilot-provisioned accounts are separate from local-review auth: operators create vendor, firm, consultant, or admin users from `/admin/users`, store only salted password hashes, and can require first-login password rotation through `/sign-in/password-update`.
