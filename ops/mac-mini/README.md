...
- `pnpm ops:mac-mini:restart` safely restarts only the app agent
- `pnpm ops:mac-mini:launchd:check` is the agent/install sanity check
- `pnpm ops:mac-mini:verify` is the heavier nightly control
- `pnpm ops:mac-mini:chatops:self-test` dry-runs the allowlisted chat commands
...
For the exact `patalign.com` production contract, use [patalign-production-launch.md](/Users/camerongarrett/work/c2acct/docs/launch/patalign-production-launch.md).
For the Telegram operator layer, use [mac-mini-chatops.md](/Users/camerongarrett/work/c2acct/docs/launch/mac-mini-chatops.md).
...
pnpm ops:mac-mini:restart
pnpm ops:mac-mini:launchd:check
pnpm ops:mac-mini:verify
pnpm ops:mac-mini:chatops:self-test
```
...
- chatops and watchdog agent load state
...
- Chat-ops uses a long-polling Telegram bot and an independent watchdog agent.
...
- Chat-ops audit events go to `artifacts/mac-mini/state/chatops-audit.jsonl`.
...
- The chat-ops install path will refuse to start if `TELEGRAM_BOT_TOKEN` or `TELEGRAM_ALLOWED_CHAT_ID` is missing.
- The app agent will also refuse to start if `AUTH_URL` does not exactly match `MAC_MINI_PUBLIC_ORIGIN` or if `MAC_MINI_HOST` is not `127.0.0.1`.
